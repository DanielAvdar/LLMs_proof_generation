...
def show_solution(problem):
    """Print problem solution details and return the Relations section as a string."""
    ...
    def append_to_relations_output(*args):
        relations_output.append(" ".join(map(str, args)) + "\n")

    """-----------Condition Declaration Statements-----------"""
    print_only("problem_index:", problem.parsed_problem_CDL["id"])
    print_only("construction_cdl:")
    for construction_fl in problem.parsed_problem_CDL["cdl"]["construction_cdl"]:
        print_only(construction_fl)
    print_only("text_cdl:")
    for text_fl in problem.parsed_problem_CDL["cdl"]["text_cdl"]:
        print_only(text_fl)
    print_only("image_cdl:")
    for image_fl in problem.parsed_problem_CDL["cdl"]["image_cdl"]:
        print_only(image_fl)
    print_only("goal_cdl:")
    print_only(problem.parsed_problem_CDL["cdl"]["goal_cdl"], "\n")

    """-----------Process of Problem Solving-----------"""
    print_only("reasoning:")
    ...
    for step in anti_parsed_cdl:
        for cdl in anti_parsed_cdl[step]:
            ...
            print_only("\033[35m" + ";".join(formatted_values).replace(" ", "") + "\033[0m")
    ...
    for predicate in basic_predicates + predicates:
        ...
        if len(items) == 0:
            continue
        append_to_relations_output(predicate + ":")
        print_only(predicate + ":")

        if predicate in basic_predicates and len(items) > 200:
            items = items[0:100] + ["..."] + items[len(items) - 100:]

        for item in items:
            if isinstance(item, str):
                append_to_relations_output(item)
                print_only(item)
                continue
            ...
            if _id not in used_pid:
                append_to_relations_output("(" + str(_id) + ";" + item + ";" + premises + ";" + theorem + ")")
                print_only("(" + str(_id) + ";" + item + ";" + premises + ";" + theorem + ")")
            else:
                append_to_relations_output("(" + str(_id) + ";" + item + ';' + premises + ";" + theorem + ")")
                print_only("(" + str(_id) + ";" + item + ';' + premises + ";" + theorem + ")")
    append_to_relations_output("\n")
    ...
    for attr in problem.condition.sym_of_attr:
        ...
        if isinstance(problem.condition.value_of_sym[sym], Float):
            line = ";".join([f"{attr[0]}({''.join(attr[1])})", str(sym), f"{problem.condition.value_of_sym[sym]:.3f}"])
        else:
            line = ";".join([f"{attr[0]}({''.join(attr[1])})", str(sym), str(problem.condition.value_of_sym[sym])])

        sym_and_val_string += line + "\n"
        print_only(line)

    print_only("Equations:")
    equations_string = ""
    ...
    for item in items:
        ...
        if _id not in used_pid:
            line = ";".join([str(_id), str(item), str(premises), str(theorem)])
        else:
            formatted_values = [
                f"{_id}",
                f"{item}",
    ...
    """-----------Solving Goal-----------"""
    print_only("Solving Goal:")
    print_only("type: {}".format(problem.goal.type))
    goal_string = "goal: {}".format(str(problem.goal.item).replace(" ", ""))
    answer_string = "answer: {}".format(str(problem.goal.answer).replace(" ", ""))
    print_only(goal_string)
    print_only(answer_string)


    if problem.goal.solved:
        print_only("solved: True")
    else:
        print_only("solved: False")
    if problem.goal.solved_answer is not None:
        print_only("solved_answer: {}".format(str(problem.goal.solved_answer)))
        if not isinstance(problem.goal.solved_answer, tuple):
            try:
                print_only("solved_answer(float): {}".format(float(problem.goal.solved_answer)))
            except TypeError:
                print_only("solved_answer(float): <Cannot convert to float>")
    if problem.goal.premise is not None:
        print_only("premise: {}".format(str(problem.goal.premise)))
        print_only("theorem: {}".format(inverse_parse_one_theorem(problem.goal.theorem, problem.parsed_theorem_GDL)))
    print_only("\n")
    ...
    for step in problem.timing:
        if problem.timing[step][0] in used_theorem:
            ...
            print_only("{:^2} {} {:.6f}s".format(step, theorem, problem.timing[step][1]))
        else:
            print_only("{:^2} {} {:.6f}s".format(step, problem.timing[step][0], problem.timing[step][1]))
        time_sum += problem.timing[step][1]
    print_only("total: {:.6f}s".format(time_sum))

    return {'relations': "".join(relations_output), 'symbols_and_values': sym_and_val_string, 'equations': equations_string, 'goal_value': goal_string, 'answer': answer_string}


def show_solution_old(problem):
    """-----------Condition Declaration Statements-----------"""
    ...
    for step in anti_parsed_cdl:
        for cdl in anti_parsed_cdl[step]:
            ...
            print("\033[35m" + ";".join(formatted_values).replace(" ", "") + "\033[0m")
    ...
    for predicate in basic_predicates + predicates:
        ...
        for item in items:
            ...
            if _id not in used_pid:
                ...
                print(str(_id) + ";" + item + ";" + premises + ";" + theorem)
            else:
    ...
    for attr in problem.condition.sym_of_attr:
        sym = problem.condition.sym_of_attr[attr]
        if isinstance(problem.condition.value_of_sym[sym], Float):
            # print("{0:^70}{1:^15}{2:^20.3f}".format(  "{}({})".format(attr[0], "".join(attr[1])), str(sym), problem.condition.value_of_sym[sym]))
            print(";".join([f"{attr[0]}({''.join(attr[1])})", str(sym), f"{problem.condition.value_of_sym[sym]:.3f}"]))

        else:
            # print("{0:^70}{1:^15}{2:^20}".format(   "{}({})".format(attr[0], "".join(attr[1])), str(sym), str(problem.condition.value_of_sym[sym])))
            print(";".join([f"{attr[0]}({''.join(attr[1])})", str(sym), str(problem.condition.value_of_sym[sym])]))
    ...
    for item in items:
        ...
        if _id not in used_pid:
            ...
            print(";".join([str(_id), str(item), str(premises), str(theorem)]))
            ...
            print("\033[35m" + ";".join(formatted_values) + "\033[0m")
